...
| **Truck Racing** (3D racing, platform-relay realtime) | https://usion-racing.vercel.app | https://github.com/ugshanyu/usion-racing · seed `backend/scripts/seed_usion_racing.py` — 2–4p 3D truck racing (three.js + crashcat physics, port of mrdoob/Starter-Kit-Racing). Client-simulated own car + 15 Hz `realtime('car', …)` broadcasts with snapshot interpolation for remote cars; car-to-car hits are symmetric local impulses (no server). Race lifecycle rides sequenced `action()`s (`kickoff` locks seats, `finished` order IS the placement order on every client, host declares `race_over` + `reportResult`). Waiting hall, quick chat, bots escape hatch, en/mn i18n, `<meta name="usion:leaderboard" content="asc">` best-lap board. Static ES modules on Vercel, no build step (Path B, loads the SDK itself). |
